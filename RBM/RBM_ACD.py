...
class RBM(object):
    ...
    def average_contrastive_divergence(self, lr=0.1, k=1, l=10, input=None, label=None):
        ...
        nv_means=[[] for i in range(l)]
        nv_samples=[[] for i in range(l)]
        nh_means=[[] for i in range(l)]
        nh_samples=[[] for i in range(l)]
        nl_means=[[] for i in range(l)]
        nl_samples=[[] for i in range(l)]

        for step in range(k):
            for av_step in range(l):
                if step == 0:
                    nv_means[av_step], nv_samples[av_step], \
                    nh_means[av_step], nh_samples[av_step], \
                    nl_means[av_step], nl_samples[av_step] = self.gibbs_hvh(chain_start)

                else:
                    nv_means[av_step], nv_samples[av_step], \
                    nh_means[av_step], nh_samples[av_step], \
                    nl_means[av_step], nl_samples[av_step] = self.gibbs_hvh(nh_samples)
        ...
        self.Wm += (numpy.dot(self.input.T/self.input_std, ph_mean) - numpy.dot(numpy.mean(nv_samples,axis=0).T/self.input_std, numpy.mean(nh_means,axis=0)))
        self.Um += (numpy.dot(self.label.T, ph_mean) - numpy.dot(numpy.mean(nl_samples,axis=0).T, numpy.mean(nh_means,axis=0)))
        self.vbm += numpy.mean(self.input/self.input_std - numpy.mean(nv_samples,axis=0)/self.input_std, axis=0)
        self.hbm += numpy.mean(ph_sample - numpy.mean(nh_means,axis=0), axis=0)
        self.lbm += numpy.mean(self.label - numpy.mean(nl_samples,axis=0), axis=0)
...
def test_rbm(dataname="new-thyroid.data", learning_rate=0.01, k=1, training_epochs=1000):
    ...
    for epoch in range(training_epochs):
        rbm.average_contrastive_divergence(lr=learning_rate, k=k)
